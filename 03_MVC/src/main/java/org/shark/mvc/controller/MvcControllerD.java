...
@RequestMapping("/d")
@SessionAttributes("boards") // 2. boards 이름의 Model 속성을 찾아 세션에 저장한다
@Controller
public class MvcControllerD {
  ...
  public String regist(@ModelAttribute(name = "boards") BoardList boardList // 3. 세션에 있는 boards를 BoardList로 가져온다
                      , BoardDTO board) {                                   // 세션에 boards가 없으면 컨트롤러의 @ModelAttribute("boards") 메소드(getBoards())가 호출되어 새로운 BoardList 객체를 생성한다
