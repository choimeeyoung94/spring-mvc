 ...
 *      (3) 이름도 일치하는 빈이 없으면 예외가 발생한다
 *      (4) 이런 문제를 피하기 위해서 @Qualifier를 명시하기도 한다         
 * 5. 스프링에서 인터페이스 타입의 빈을 주입할 때는 "인터페이스 타입"으로 선언하는 것이 원칙이며 권장 사항이다
 * 6. 의존성 주입 대상
 *    1) Field (필드)          코드 간단. 불변성 약화(final 불가)
 *    2) Setter (수정자)       빈이 없어도 동작 가능(런타임 오류 발생) 
...
//  @Autowired
//  public void setDIService(DIService diService) { // 파라미터 DIService diServce로 주입받는다
...
//  }
   ...
   // 생성자가 한 개인 경우 생략할 수 있다
   public MainController(DIService diService) { // 파라미터 DIService diServce로 주입받는다
     this.diService = diService;
   }
