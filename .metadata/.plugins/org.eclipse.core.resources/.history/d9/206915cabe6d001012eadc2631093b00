...
public class MvcControllerC {
   ...
   *  3. 용도
   *     1) 컨트롤러 메소드 : 모든 요청 메소드 이전에 먼저 실행되어 공통적으로 Model에 데이터를 저장한다
   *     2) 파라미터 : 요청 파라미터( 또는 폼 데이터)를 자바 객체에 자동으로 바인딩한다
   *  
   *  
   *  
  ...
  public String commonAttr() {
    return "공통값"; // 모든 View(JSP)에서 ${common}으로 접근할 수 있다
  }
  
  @RequestMapping("/submit.do") // 요청 주소 : /c/submit.do?title=제목&hit=0
  public String submit(@ModelAttribute(name = "board") BoardDTO board) { // Model에 board라는 이름으로 BoardDTO board 객체가 저장된다
                                                                         // name = "board"를 생략하면 (@ModelAttribute BoardDTO board) model에 boardDTO(타입을 이름으로 사용) 라는 이름으로 BoardDTO board 이름이 저장된다 => JSP에서 ${boardDTO}라는 이름으로 확인
                                                                         // @ModelAttribute를 생략할 수 있다 (스프링 3.2 이후)이 경우 Model에 boardDTO라는 이름을 저장
    return "c/detail";
  }
